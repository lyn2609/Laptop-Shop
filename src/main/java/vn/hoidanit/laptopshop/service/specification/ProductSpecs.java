...
public class ProductSpecs {
    ...
    public static Specification <Product> matchListRam(List <String> ram) {
        return (root, query, criteriaBuilder) -> {
            jakarta.persistence.criteria.Predicate[] predicates = ram.stream()
                .map(r -> criteriaBuilder.like(
                    criteriaBuilder.lower(root.get("ram")),
                    "%" + r.toLowerCase() + "%"))
    ...
    public static Specification <Product> matchListCpu(List <String> cpu) {
        return (root, query, criteriaBuilder) -> {
            jakarta.persistence.criteria.Predicate[] predicates = cpu.stream()
                .map(c -> criteriaBuilder.like(
                    criteriaBuilder.lower(root.get("cpu")),
                    "%" + c.toLowerCase() + "%"))
                .toArray(jakarta.persistence.criteria.Predicate[]::new);
            return criteriaBuilder.or(predicates);
        };
